...
class TripleTable:
    ...
    def tripleNewtonInter(self, x, y, z, nx, ny, nz, showComments=True):
        indZStart, indZEnd = findNearestInd(self.z, z, nz) #--соседние точки по z
        if showComments:
        ...
        for k in range(indZStart, indZEnd):
            ...
            #- собираем значения по интерполяции

        if showComments:
            print('ZTable =', ZTable)

        # теперь последняя интерполяция по z и ZTable
        tableForNewton = [[self.z[i] for i in range(indZStart, indZEnd)], ZTable] #-- матрица из
                                                    #-- массвиа точек по z и ZTable
